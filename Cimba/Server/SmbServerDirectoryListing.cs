    ...
    public class SmbServerDirectoryListing
    {
        ...
        internal static byte[] Flatten(List<SmbServerDirectoryListing> listings)
        {
            ...
            for (int i = 0; i < listings.Count; i++)
            {
                ...
                BitConverterLE.GetBytes(nextEntryOffset).CopyTo(bytearrays[i], 0);
                BitConverterLE.GetBytes((ulong)listings[i].CreationTime.ToFileTime()).CopyTo(bytearrays[i], 8);
                BitConverterLE.GetBytes((ulong)listings[i].LastAccessTime.ToFileTime()).CopyTo(bytearrays[i], 16);
                BitConverterLE.GetBytes((ulong)listings[i].LastWriteTime.ToFileTime()).CopyTo(bytearrays[i], 24);
                BitConverterLE.GetBytes((ulong)listings[i].ChangeTime.ToFileTime()).CopyTo(bytearrays[i], 32);
                BitConverterLE.GetBytes((ulong)listings[i].EndofFile).CopyTo(bytearrays[i], 40);
                BitConverterLE.GetBytes((ulong)listings[i].AllocationSize).CopyTo(bytearrays[i], 48);
                BitConverterLE.GetBytes((ulong)ConvertFileAttributes(listings[i].Attributes)).CopyTo(bytearrays[i], 56);
                BitConverterLE.GetBytes((uint)Encoding.Unicode.GetByteCount(listings[i].FileName)).CopyTo(bytearrays[i], 60);
