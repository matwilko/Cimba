namespace Cimba.Protocol
{
    ...
    internal class SessionSetupRequest : Packet
    {
        ...
        internal static SessionSetupRequest Read(MemoryStream stream)
        {
            ...
            ushort ss = BitConverterLittleEndian.ToUShort(stream);
            if (ss != 24 && ss != 25)
            {
            ...
            packet.SupportsDFS = (BitConverterLittleEndian.ToUInt(stream) & 0x02) == 0x02;

            // Channel (4 bytes) - field MUST NOT be used and MUST be reserved - MUST ignore
            stream.Seek(4, SeekOrigin.Current);

            // SecurityBufferOffset (2 bytes)
            int bufferOffset = BitConverterLittleEndian.ToUShort(stream);

            // SecurityBufferLength (2 bytes)
            int bufferLength = BitConverterLittleEndian.ToUShort(stream);

            // PreviousSessionId (8 bytes)
            packet.PreviousSessionId = BitConverterLittleEndian.ToULong(stream);
        ...
        protected override byte[] Generate()
        {
            ...
            // StructureSize (2 bytes)
            BitConverterLittleEndian.GetBytes((ushort)24).CopyTo(data, 0);
            ...
            // SecurityMode (1 byte)
            BitConverterLittleEndian.GetBytes((byte)(ushort)this.SecurityMode).CopyTo(data, 3);

            // Capabilities (4 bytes)
            BitConverterLittleEndian.GetBytes((uint)(this.SupportsDFS ? 0x00000001 : 0x00000000)).CopyTo(data, 4);

            // Channel (4 bytes) - MUST NOT be used and MUST be reserved

            // SecurityBufferOffset (2 bytes) - offset from beginning of SMB2 Header
            BitConverterLittleEndian.GetBytes((ushort)(Packet.HeaderLength + 24)).CopyTo(data, 12);

            // SecurityBufferLength (2 bytes)
            BitConverterLittleEndian.GetBytes((ushort)this.SecurityBuffer.Length).CopyTo(data, 14);

            // PreviousSessionId (8 bytes)
            BitConverterLittleEndian.GetBytes(this.PreviousSessionId).CopyTo(data, 16);
