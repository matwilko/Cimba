namespace Cimba.Protocol
{
    ...
    internal class NegotiateRequest : Packet
    {
        ...
        internal static NegotiateRequest Read(MemoryStream stream)
        {
            ...
            if (BitConverterLittleEndian.ToUShort(stream) != 36)
            {
            ...
            ushort dialectCount = BitConverterLittleEndian.ToUShort(stream);
            if (dialectCount == 0)
            {
                // TODO: Return error packet
                throw new SmbPacketException("Invalid NegotiateRequest");
            ...
            packet.SecurityMode = (Negotiate_SecurityMode)BitConverterLittleEndian.ToUShort(stream);
            ...
            {
                dialects[i] = BitConverterLittleEndian.ToUShort(stream);
            }
        ...
        protected override byte[] Generate()
        {
            ...
            BitConverterLittleEndian.GetBytes((ushort)36).CopyTo(output, 0);

            // DialectCount (2 bytes) - The number of dialects in the Dialects[] array
            BitConverterLittleEndian.GetBytes((ushort)this.Dialects.Length).CopyTo(output, 2);

            // SecurityMode (2 bytes) - Specifies whether signing is enabled, required at the server, or both.
            BitConverterLittleEndian.GetBytes((ushort)(this.RequireSigning ? Negotiate_SecurityMode.SigningRequired : Negotiate_SecurityMode.SigningEnabled)).CopyTo(output, 4);
            ...
            BitConverterLittleEndian.GetBytes((ushort)0).CopyTo(output, 8);
            ...
            BitConverterLittleEndian.GetBytes((ulong)0).CopyTo(output, 28);
            ...
            {
                BitConverterLittleEndian.GetBytes((ushort)this.Dialects[i]).CopyTo(output, startindex);
                startindex += 2;
            }
